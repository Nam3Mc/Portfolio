...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError("")
    if (!isFormValid) return setError("Revisa los campos")
  ...
  const fillDemo = (type: "owner" | "guest") => {
    setEmail(type === "owner" ? "alice@example.com" : "carlos@example.com")
    setPassword("123456")
  }
    ...
    <div className="min-h-screen grid md:grid-cols-2">
      ...
      <div className="hidden md:flex relative flex-col justify-between overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 text-white p-10">
        ...
        <div className="relative z-10">
          <h1 className="text-2xl font-semibold tracking-tight">Renta Fácil</h1>
        </div>
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-3 text-sm">
            {["Escrow automático", "Contrato digital verificable", "Pagos en USDC"].map((f, i) => (
          ...
          <div className="relative mt-6">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 shadow-xl">
              <p className="text-xs text-indigo-200 mb-2">Pago protegido</p>
              <div className="flex justify-between items-center">
                <span className="text-sm">Contrato activo</span>
                <span className="text-xs bg-green-400/20 text-green-200 px-2 py-1 rounded-full">Escrow</span>
              </div>
        ...
        {/* FOOTER */}
        <div className="relative z-10 text-xs text-indigo-200">© 2026 Renta Fácil</div>
      </div>
      ...
      <div className="flex items-center justify-center px-4 py-10 bg-gray-50">
        <div className="w-full max-w-md bg-white p-6 sm:p-8 rounded-2xl shadow-xl">
          <h1 className="text-2xl font-semibold mb-2 text-gray-800">Bienvenido 👋</h1>
          <p className="text-gray-500 mb-6 text-sm">Inicia sesión para continuar</p>

          {/* DEMO USERS */}
          <div className="flex gap-2 mb-4">
            <button onClick={() => fillDemo("owner")} className="text-xs px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full hover:bg-indigo-200">
              Demo Owner
            </button>
            <button onClick={() => fillDemo("guest")} className="text-xs px-3 py-1 bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200">
              Demo Guest
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <InputField
              ...
              setTouched={() => setTouched({ ...touched, email: true })}
            ...
            <InputField
              ...
              setTouched={() => setTouched({ ...touched, password: true })}
          ...
          >
            ...
            <span className="relative text-gray-700 group-hover:text-gray-900">Continuar con Google</span>
          </button>
...
/* 👇 Reusable input field con icono y validación */
...
function InputField({ type, placeholder, value, onChange, icon, touched, setTouched, isValid, errorMsg, togglePassword, showPassword }: InputProps) {
  return (
    <div className="relative">
      ...
      {touched && !isValid && <p className="text-xs text-red-500 mt-1">{errorMsg}</p>}
    </div>
  )
}